    ...
    ngsiUtils.updateEntity(
        ...
        function handleUpdateEntity(error, response, body) {
            if (error) {
                ...
                logger.error(context, 'Transport error connecting with the Context Broker: ' + error);
            } else if (body && body.errorCode) {
                logger.debug(context, 'Application error connecting with the Context Broker:\n\n%j\n', body);
            } else {
...
function updateRegistration(object, payload, callback) {
        ...
        lwm2mLib.listObservers(function observersHandler(error, observersList) {
            if (error) {
                ...
                for (var i = 0; i < observersList.length; i++) {
                    if (observersList[i].deviceId.toString() === object.id.toString()) {
    ...
    async.waterfall([
        ...
        apply(iotAgentLib.getDevice, object.name + ':' + object.type),
        apply(observeActiveAttributes, payload)
    ], callback);
}
