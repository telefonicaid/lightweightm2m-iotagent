...
var iotAgentLib = require('iotagent-node-lib'),
    ...
    omaInverseRegistry = require('../omaRegistryInverseMap'),
...
function registrationHandler(endpoint, lifetime, version, binding, payload, callback) {
    ...
    function findConfiguration(device, callback) {
        var deviceConfiguration,
            cleanResource = device.path.substr(0, device.path.lastIndexOf('/'));
        ...
        iotAgentLib.getConfiguration(cleanResource, '', function(error, configuration) {
            if (!error && configuration) {
                callback(null, device, configuration);
            } else if (device.type && config.ngsi.types[device.type]) {
                callback(null, device, config.ngsi.types[device.type]);
            } else {
    ...
    function addUnsupportedAttributes(deviceInformation, callback) {
        ...
        if (deviceInformation.type &&
            config.ngsi.types[deviceInformation.type] &&
            config.ngsi.types[deviceInformation.type].lwm2mResourceMapping) {
