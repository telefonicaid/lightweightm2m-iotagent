 ...
 */
function addUnsupportedAttributes(payload, deviceInformation, callback) {
    ...
    if (deviceInformation.type &&
        commons.getConfig().ngsi.types[deviceInformation.type] &&
        commons.getConfig().ngsi.types[deviceInformation.type].lwm2mResourceMapping) {
        mappings = _.values(commons.getConfig().ngsi.types[deviceInformation.type].lwm2mResourceMapping);
    } else {
        mappings = [];
    }
...
function createNewDevice(lwm2mDevice, configuration, payload, innerCallback) {
    async.waterfall([
        ...
        apply(addUnsupportedAttributes, payload),
