...
function observeActiveAttributes(payload, registeredDevice, callback) {
    ...
    for (var i = 0; i < activeAttributes.length; i++) {
        // Ezequiel: esto debe ser nulo, y todos los if han sido modificados
        var lwm2mMapping = null;
        ...
        if (!lwm2mMapping && commons.getConfig().ngsi.types[registeredDevice.type]) {
            lwm2mMapping = commons.getConfig()
                .ngsi
                .types[registeredDevice.type]
                .lwm2mResourceMapping[activeAttributes[i].name];
        }
        if (!lwm2mMapping && omaInverseRegistry[activeAttributes[i].name]) {
