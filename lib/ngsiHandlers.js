...
function extractOperations(device, attribute) {
  ...
  if (
    ...
    operations =
      device.internalAttributes.lwm2mResourceMapping[attribute].operations;
  }
  if (
    !operations &&
    ...
    operations =
      config.ngsi.types[device.type].lwm2mResourceMapping[attribute].operations;
  }
...
function ngsiUpdateHandler(
  ...
  logger.debug(
    context,
    'Handling device data update from the north port for device [%s] of type [%s]',
    id,
    type
  );
  ...
  iotAgentLib.getDeviceByName(id, service, subservice, function(
    error,
    ngsiDevice
  ) {
    if (error) {
...
function ngsiQueryHandler(id, type, service, subservice, attributes, callback) {
  ...
  logger.debug(
    context,
    'Handling device data query from the north port for device [%s] of type [%s]',
    id,
    type
  );
  logger.debug(context, 'New attributes;\n%s', attributes);
