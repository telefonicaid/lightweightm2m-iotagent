...
function sendLwm2mOperation(operation, deviceId, type, attributeName, attributeValue, callback) {
    var mapping = extractMapping(type, attributeName),
        parameters;
...
function readAttribute(deviceId, type, attribute, innerCallback) {
    sendLwm2mOperation(lwm2mLib.read, deviceId, type, attribute, null, innerCallback);
}

function writeAttribute(deviceId, type, attribute, innerCallback) {
    sendLwm2mOperation(lwm2mLib.write, deviceId, type, attribute.name, attribute.value, innerCallback);
}

function executeAttribute(deviceId, type, attribute, innerCallback) {
    sendLwm2mOperation(lwm2mLib.execute, deviceId, type, attribute.name, attribute.value, innerCallback);
}
...
function ngsiExecuteHandler(id, type, attributes, callback) {
    ...
    logger.debug(context, 'Handling device command execution from the northbound for device [%s] of type [%s]', id, type);
    logger.debug(context, 'Arguments: \n%s', attributes);

    function executeCommand(device, innerCallback) {
        async.map(
            attributes,
            async.apply(executeAttribute, device.id, device.type),
            innerCallback);
    }
